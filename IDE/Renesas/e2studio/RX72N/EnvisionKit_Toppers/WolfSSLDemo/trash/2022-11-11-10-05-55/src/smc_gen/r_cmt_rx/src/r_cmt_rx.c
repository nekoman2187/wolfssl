...
#elif (BSP_CFG_RTOS_USED == 4) ||  (BSP_CFG_RTOS_USED == 6)   /* Renesas RI600V4 & RI600PX */
...
static bool cmt_create (uint32_t frequency_hz, void (* callback)(void * pdata), cmt_modes_t mode, uint32_t * channel)
{
    ...
    else
    {
        ...
        ret = cmt_setup_channel(*channel, frequency_hz); //wata
      //    ret = true; // wata
        ...
        {        
            ...
            cmt_counter_start(*channel); //wata
        }
        else
        {
    ...
    if (((uint32_t)CMT_PCLK_HZ / 8) >= frequency_hz)
    {
        ...
        for (i = 0; i < ((sizeof(g_cmt_clock_dividers))/(sizeof(g_cmt_clock_dividers[0]))); i++)
        {
            ...
            if (frequency_hz > (((uint32_t)CMT_PCLK_HZ / g_cmt_clock_dividers[i]) / (uint32_t)CMT_RX_MAX_TIMER_TICKS))
            {
                ...
                power_on(channel);
                uint32_t cmt1 = CMT_PCLK_HZ;
                uint32_t cmt2 = g_cmt_clock_dividers[i];
				uint32_t cmt3 = frequency_hz;
...
***********************************************************************************************************************/
#if BSP_CFG_RTOS_USED == 0      /* Non-OS */
 R_BSP_PRAGMA_STATIC_INLINE(cmt_isr_common) // wata
...
#elif BSP_CFG_RTOS_USED == 6    /* Toppers */
     void cmt_isr_common (uint32_t channel);
...
***********************************************************************************************************************/
#if BSP_CFG_RTOS_USED == 0      /* Non-OS */
R_BSP_PRAGMA_STATIC_INTERRUPT (cmt0_isr,VECT(CMT0, CMI0)) //wata

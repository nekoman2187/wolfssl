...
*         : 12.12.2017 2.07    Fixed UDP behavior.
...
extern const UW _tcp_keepalive_start[];
extern const UW _tcp_keepalive_interval[];
extern const UW _tcp_keepalive_count[];
...
extern void get_hash_value(UB lan_port_no, UB *message, UW message_len, UB **hash, UW *hash_len);
...
void _tcp_init_tcb(_TCB *_ptcb)
{
    ...
    ch = tcp_ccep[_ptcb->cepid - 1].lan_port_number;
    ...
    _tcp_keepalive_timer_refresh(_ptcb);
    _ptcb->fin_wait2_rst_cnt = 0xffff;
#endif
    ...
    if (_process_flag == 0)
    {
        ...
        while (1)
        {
            ...
            if (1 == _t4_dhcp_enable)
            {
                ...
                {
                    channel = tcp_ccep[cnt].lan_port_number;
                ...
                {
                    channel = udp_ccep[cnt].lan_port_number;
                    if (1 == (_ch_info_head[channel].ip_terminated_flag))
                    {
            ...
            {
                if ((_ch_info_head[counter]._rcvd == 1) && ((_ch_info_head[counter].flag & _CHINFOF_SND_ICMP) == 0))
                {
                    break;
                }
            }
...
void _proc_api(void)
{
    ...
    for (counter = 0; counter < __tcpcepn; counter++)
    {
        ...
        _ch_info_tbl = &_ch_info_head[tcp_ccep[counter].lan_port_number];
...
void _tcp_api_acpt(void)
{
    ...
    _tcp_tcb->tcp_initial_seqno = tcpudp_get_time() + generate_sequence_number(_ch_info_tbl, _tcp_tcb);
    _tcp_tcb->suna = _tcp_tcb->tcp_initial_seqno;
    _tcp_tcb->snxt = _tcp_tcb->tcp_initial_seqno;
...
void _tcp_api_con(void)
{
    ...
    {
        get_random_number((UB*)&ui_rndm, sizeof(ui_rndm));  /* get a random number for local port number */
...
void _tcp_api_sht_cls(void)
{
    ...
    switch (_tcp_tcb->status)
    {
        ...
        case _TCPS_FIN_WAIT2:
            _tcp_tcb->fin_wait2_rst_cnt = _tcp_rt_tmo_rst[_ch_info_tbl->_ch_num];
            return ;
        default :
            return ;
    }
...
void _tcp_api_snddt(void)
{
    ...
    if (_tcp_dack[ tcp_ccep[ _tcp_tcb->cepid - 1].lan_port_number ] == 1)
    {
            ...
            case(_IPH_TCP):
                ...
                while (counter != __tcpcepn)
                {
                    ...
                    if ((ptcph->sport == hs2net(_tcp_tcb->rem_port))
                            ...
                            && (_ch_info_tbl->_ch_num == tcp_ccep[counter].lan_port_number)
                            && (  (_tcp_tcb->status != _TCPS_LISTEN)
                                  || ((_tcp_tcb->status == _TCPS_LISTEN) && \
...
ER _tcp_rcv_syn(void)
{
    ...
    switch (_tcp_tcb->status)
    {
        ...
        case _TCPS_LISTEN:
            ...
            seq = _tcp_tcb->tcp_initial_seqno;
            _tcp_tcb->suna  = seq;
            _tcp_tcb->snxt  = seq;
...
void _tcp_rcv_ack(void)
{
    ...
    _tcp_keepalive_timer_refresh(_tcp_tcb);
...
void _proc_snd(void)
{
    ...
    if (_tcp_pre_timer_cnt != _tcp_timer_cnt)
    {
        ...
        for (counter = 0; counter < __tcpcepn; counter++)
        {
            ...
            if (0 != (_tcp_tcb->status & (_TCPS_ESTABLISHED | _TCPS_CLOSE_WAIT | _TCPS_FIN_WAIT2)))
            {
                if(
                    !(   ((cur_req_type == _TCP_API_SNDDT) || (cur_req_type == _TCP_API_CLSCP))
                         && ((cur_req_stat == _TCP_API_STAT_UNTREATED) || (cur_req_stat == _TCP_API_STAT_INCOMPLETE)))
                )
                {
                    if(_tcp_tcb->keepalive_interval != 0xffffffff)
                    {
                        if (_tcp_tcb->keepalive_interval > 0)
                        {
                            _tcp_tcb->keepalive_interval--;
                        }
                    }
                }
            }

            if (_tcp_tcb->fin_wait2_rst_cnt != 0xffff)
            {
                if(_tcp_tcb->fin_wait2_rst_cnt > 0)
                {
                    _tcp_tcb->fin_wait2_rst_cnt--;
                }
            }
            ...
            else if ((_tcp_tcb->retrans_q.rst_cnt == 0) || (_tcp_tcb->fin_wait2_rst_cnt == 0))
            {
                if (cur_req_type != 0)
                {
                    switch (cur_req_type)
                ...
                _tcp_tcb->fin_wait2_rst_cnt     = 0xffff;
            }
            ...
            else if (_tcp_tcb->keepalive_interval == 0)
            {
                /* KEEP ALIVE */
                _tcp_tcb->keepalive_count --;
                if(_tcp_tcb->keepalive_count == 0xffffffff)
                {
                    /* RST */
                    _tcp_tcb->keepalive_interval = 0xffffffff;
                    *(_tcp_tcb->req.error) = E_CLS;
                    _tcp_tcb->nxt_status  = _TCPS_CLOSED;
                    _tcp_tcb->status  = _TCPS_CLOSED;
                    _tcp_tcb->it_stat  = _ITS_RST;
                ...
                {
                    /* KEEP ALIVE */
                    _tcp_tcb->keepalive_interval = _tcp_keepalive_interval[_ch_info_tbl->_ch_num];
                    _tcp_tcb->flag |= (_TCBF_NEED_SEND | _TCBF_SND_KEEPALIVE);
                }
            }
        }
...
#if defined(_ETHER)
        if (_ch_info_tbl->flag & _CHINFOF_SND_ARP_REP)
        {
#if defined(__GNUC__)
            _ether_snd_arp(ae);
#else
...
#endif
#if defined(_ICMP)
        if (_ch_info_tbl->flag & _CHINFOF_SND_ICMP)
        {
            /* Casting the pointer to a _IP_PKT type is valid. */
            pip  = (_IP_PKT *)(_ch_info_tbl->_p_rcv_buf.pip);

            /* Casting the pointer to a _ICMP_PKT type is valid. */
            picmp = (_ICMP_PKT *)(pip->data);
            picmpdata = picmp->data;
            picmph_r = &picmp->icmph;
            /* Casting the pointer to a uint16 type is valid. */
            len = ((net2hs(pip->iph.ip_total_len) - _ICMP_HLEN) - _IP_HLEN_MIN);

            /* Casting the pointer to a _ICMP_HDR type is valid. */
            ...
            /* Casting the pointer to a uchar type is valid. */
    ...
    for (counter = 0; counter != __tcpcepn; counter++)
    {
        ...
        if (_tcp_tcb->flag & _TCBF_NEED_SEND)
        {
            if (_tcp_tcb->flag & (_TCBF_SND_TCP | _TCBF_SND_RTX | _TCBF_SND_ZWIN | _TCBF_SND_KEEPALIVE))
            {
...
void _tcp_snd(void)
{
    ...
    else
    {
        ...
        else if (_tcp_tcb->flag & _TCBF_SND_KEEPALIVE)
        {
            pdata = _tcp_tcb->nxtdat;
            seq  = _tcp_tcb->snxt - 1;
            flg  |= _TCPF_ACK;
...
void _tcp_clr_rtq(_TCB *_ptcb)
{
    ...
    if (_tcp_dack[ tcp_ccep[_ptcb->cepid - 1].lan_port_number ] == 1)
    {
...
/***********************************************************************************************************************
* Function Name: _tcp_keepalive_timer_refresh
* Description  : Refresh Keep-alive packet send timer
* Arguments    : _TCB *_ptcb
* Return Value : none
***********************************************************************************************************************/
void _tcp_keepalive_timer_refresh(_TCB *_ptcb)
{
    UB channel = tcp_ccep[_ptcb->cepid - 1].lan_port_number;

    if(0 == tcp_ccep[_ptcb->cepid - 1].keepalive_enable)
    {
        /* keep alive is disabled */
        _ptcb->keepalive_interval = 0xffffffff;
    }
    else
    {
        /* keep alive is enabled */
        _ptcb->keepalive_interval = _tcp_keepalive_start[channel];
    }
    _ptcb->keepalive_count = _tcp_keepalive_count[channel];
}
#endif /* _TCP */

/***********************************************************************************************************************
* Function Name: generate_sequence_number
* Description  : Pseudorandom function for initial sequence number (ISN). 
*                Refer to RFC 6528.
*                  => ISN = M + F(localip, localport, remoteip, remoteport, secretkey)
*                      M: Count-uping time by 10ms
*                      F: Pseudorandom function (PRF); this function
*                      localip: local ip address
*                      localport: tcp local port
*                      remoteip : remote ip address
*                      remoteport : tcp remote port
*                      secretkey: With TRNG case: Ether MAC address[6] xor get_random_number[16] (generated by TRNG outside of this library)
*                                 Without TRNG case: Ether MAC address[6] xor get_random_number[16] (generated by PRNG outside of this library)
* Arguments    : _CH_INFO *_pch_info, TCB *_ptcb
* Return Value : The value generated by pseudorandom function
***********************************************************************************************************************/
UW generate_sequence_number(_CH_INFO *_pch_info, _TCB *_ptcb)
{
    UB secretkey[16];
    UW localip = _pch_info->_myipaddr[0];
    UH localport = hs2net(_ptcb->loc_port);
    UB *remoteip = _tcp_tcb->rem_ip;
    UH remoteport = hs2net(_ptcb->rem_port);

    UB hash_message_data[HASH_MESSAGE_LENGTH_FOR_SEQ_NUM];
    UB *tmp_p;
    UB *hash_result;
    UW hash_result_len;
    UW seq = 0;
    ...
    get_random_number(secretkey, sizeof(secretkey));
    for(i = 0; i < sizeof(_myethaddr[_pch_info->_ch_num]); i++)
    {
        secretkey[i] ^= _myethaddr[_pch_info->_ch_num][i];
    }
    
    /* Concatinate localip, localport, remoteip, remoteport, and secretkey for hassh message */
    tmp_p = &hash_message_data[0];
    memcpy(tmp_p, &localip, LOCALIP_SIZE_FOR_SEQ_NUM);
    tmp_p += LOCALIP_SIZE_FOR_SEQ_NUM;
    memcpy(tmp_p, &localport, LOCALPORT_SIZE_FOR_SEQ_NUM);
    tmp_p += LOCALPORT_SIZE_FOR_SEQ_NUM;
    memcpy(tmp_p, remoteip, REMOTEIP_SIZE_FOR_SEQ_NUM);
    tmp_p += REMOTEIP_SIZE_FOR_SEQ_NUM;
    memcpy(tmp_p, &remoteport, REMOTEPORT_SIZE_FOR_SEQ_NUM);
    tmp_p += REMOTEPORT_SIZE_FOR_SEQ_NUM;
    memcpy(tmp_p, secretkey, SECRETKEY_SIZE_FOR_SEQ_NUM);

    /* Calculate hash value for secretkey, localip, localport, remoteip, and remoteport */
    get_hash_value(_pch_info->_ch_num ,&hash_message_data[0], HASH_MESSAGE_LENGTH_FOR_SEQ_NUM, &hash_result, &hash_result_len);
    for (i = 0; i < (hash_result_len / sizeof(seq)) -1; i+=sizeof(seq)) /* -1 means avoiding final block calculating */
    {
#if (BIGENDIAN)
        seq ^= ((uint32_t)hash_result[(i * sizeof(seq)) + 0] << 24 | \
                (uint32_t)hash_result[(i * sizeof(seq)) + 1] << 16 | \
                (uint32_t)hash_result[(i * sizeof(seq)) + 2] <<  8 | \
                (uint32_t)hash_result[(i * sizeof(seq)) + 3] <<  0);
#else
        seq ^= ((uint32_t)hash_result[(i * sizeof(seq)) + 0] <<  0 | \
                (uint32_t)hash_result[(i * sizeof(seq)) + 1] <<  8 | \
                (uint32_t)hash_result[(i * sizeof(seq)) + 2] << 16 | \
                (uint32_t)hash_result[(i * sizeof(seq)) + 3] << 24);
#endif
    }

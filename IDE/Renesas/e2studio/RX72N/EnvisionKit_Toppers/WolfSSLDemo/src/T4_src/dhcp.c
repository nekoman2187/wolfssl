...
*         : 10.12.2018 2.08    Fixed random number conflict.
*         : 20.06.2019 2.09    Fixed bug: the callback routine is called when
*         :                    UDP packet is received before IP address is bound.
***********************************************************************************************************************/
/***********************************************************************************************************************
...
static ft_dhcp_handler* const gpft_dhcpf[DHCP_EVENT_COUNT + 1][DHCP_STATE_COUNT + 1] =
{
    {
...
static void dhcp_apipa(dhcp_inf_t* pt_dhcpinf)
{
    ...
    while ((uc_rnd < 1) || (254 < uc_rnd)); /* WAIT_LOOP */
...
void dhcp_rcv_dat_int(_UDP_CB *pucb, _UDP_HDR *udph, _TCPUDP_PHDR *ph)
{
    ...
    if ((DHCP_HEAD_OP_BOOTREPLY != (pt_dhcp->uc_op)) || ((hl2net(pt_dhcpinf->ul_xid_mem)) != (pt_dhcp->ul_xid))
            || (0 != memcmp(pt_dhcp->uca16_chaddr, ((_myethaddr[(pt_dhcpinf->ul_channel)])), EP_ALEN)))
    {
        /* Do Nothing */
    }
    else
...
static uint8_t* dhcp_option_detect(uint8_t* puc_opt, uint8_t uc_command)
{
    ...
    if (DHCP_PKTFLD_MAGIC_COOKIE == (net2hl(ul_magic_cookie)))
    {
        ...
        while ((DHCP_PKTFLD_MINIMUMOPTION > ul_rest_size) && (0 == ul_fin));    /* WAIT_LOOP */
    }
    else
    {
        puc_res = (void*)0;
...
/***********************************************************************************************************************
* Function Name: dhcp_ip_available_check
...
*              : -1: not available
***********************************************************************************************************************/
int32_t dhcp_ip_available_check(UB channel)
{
    dhcp_inf_t* pt;
    int32_t ret = -1;
    if(_t4_dhcp_enable == 0)
    {
        ret = 0;
    }
    else
    {
        pt = _ch_info_head[channel].pt_dhcp_inf;
        if(pt->ul_state == DHCP_STATE_BOUND || pt->ul_state == DHCP_STATE_APIPA || \
                pt->ul_state == DHCP_STATE_RENEWING || pt->ul_state == DHCP_STATE_RENEWING_WAIT)
        {
            if(0 == (pt->ul_event & DHCP_EVENT_LINK_ON))
            {
                ret = 0;
            }
        }

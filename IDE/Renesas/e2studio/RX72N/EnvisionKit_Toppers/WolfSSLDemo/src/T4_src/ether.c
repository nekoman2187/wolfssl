...
*         : 20.06.2019 2.09    Fixed bug: Subnetmask filtering.
***********************************************************************************************************************/
...
void _ether_rcv_arp(void)
{
    ...
    switch (rpap->ar_op)
    {
        ...
        case(hs2net(AR_REQUEST)):
            /* transmit flag on */
            _ch_info_tbl->flag |= (_TCBF_NEED_SEND | _CHINFOF_SND_ARP_REP);
...
sint16 _ether_snd_arp(_ARP_ENTRY *ae)
{
    ...
    if (_ch_info_tbl->flag & _CHINFOF_SND_ARP_REQ)
    {
        /* generate ARP packet */
    ...
    else if (_ch_info_tbl->flag & _CHINFOF_SND_ARP_REP)
    {
        _cpy_eaddr(parp, rpap);
        parp->ar_op = hs2net(AR_REPLY);
        memcpy(parp->ar_sha, _myethaddr[_ch_info_tbl->_ch_num], EP_ALEN);
        memcpy(parp->ar_spa, _ch_info_tbl->_myipaddr, IP_ALEN);
        memcpy(parp->ar_tha, rpap->ar_sha, EP_ALEN);
        memcpy(parp->ar_tpa, rpap->ar_spa, IP_ALEN);
    ...
    _ch_info_tbl->flag &= ~(_CHINFOF_SND_ARP_REQ | _CHINFOF_SND_ARP_REP);
...
sint16 _ether_snd_ip(uchar *data, uint16 dlen)
{
    ...
    /* 255.255.255.255 (broadcast) or (directed broadcast) */
             ...
             (0 != (((addr & ~subnet_mask) == (broad_cast_addr & ~subnet_mask)) && ((addr & subnet_mask) == (myipaddr & subnet_mask)))))
    {
    ...
    else
    {
        ...
        _ch_info_tbl->flag |= _CHINFOF_SND_ARP_REQ;
        _ether_snd_arp(ae);
...
sint16 _ether_snd(uint16 type, uchar *data, uint16 dlen)
{
    ...
    if (type == EPT_IP)
    {
        ...
        net2hl_yn_xn(&myipaddr, tcpudp_env[_ch_info_tbl->_ch_num].ipaddr);
        /* (directed broadcast) */
        if (0 != ((addr & ~subnet_mask) == (broad_cast_addr & ~subnet_mask)) && ((addr & subnet_mask) == (myipaddr & subnet_mask)))
        {
            memcpy(peh->eh_dst, eth_broadcast, EP_ALEN);
        }
    }

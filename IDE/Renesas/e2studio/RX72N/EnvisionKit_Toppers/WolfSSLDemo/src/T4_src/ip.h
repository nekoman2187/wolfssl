...
#define _CHINFOF_PEND_ICMP   0x0004
#define _CHINFOF_SND_ICMP    0x0100
#define _CHINFOF_SND_ARP_REQ 0x1000
#define _CHINFOF_SND_ARP_REP 0x2000
...
typedef struct
{
    ...
    uchar   opt[6];
#if defined(__GNUC__)
} __attribute__ ((packed)) _ICMP_NBR_PKT;
#else
} _ICMP_NBR_PKT;
#endif
...
typedef struct
{
    ...
    uchar   opt[6];
#if defined(__GNUC__)
} __attribute__ ((packed)) _ICMP_RTR_SOL_PKT;
#else
} _ICMP_RTR_SOL_PKT;
#endif
...
typedef struct
{
    ...
    uchar   prefix[16];
#if defined(__GNUC__)
} __attribute__ ((packed)) _ICMP_OPT_PFX;
#else
} _ICMP_OPT_PFX;
#endif
...
#if defined(__GNUC__)
} __attribute__ ((packed)) _ICMP_RTR_ADV_PKT;
#else
} _ICMP_RTR_ADV_PKT;
#endif
...
#if defined(__GNUC__)
} __attribute__ ((packed)) _ICMP_ECHO_PKT;
#else
} _ICMP_ECHO_PKT;
#endif
...
typedef struct _icmp_hdr
{
    ...
    uint16  seq;       /* sequence number */
#if defined(__GNUC__)
} __attribute__ ((packed)) _ICMP_HDR;
#else
} _ICMP_HDR;
#endif
...
typedef struct
{
    ...
    uchar     data[1];
#if defined(__GNUC__)
} __attribute__ ((packed)) _ICMP_PKT;
#else
} _ICMP_PKT;
#endif
